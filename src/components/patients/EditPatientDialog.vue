    ...
    <div>
        ...
        <Dialog :open="isOpen" @update:open="onOpenChange" class="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
                ...
                <DialogDescription>Atualize as informações do paciente {{ patient?.name }}.</DialogDescription>
            </DialogHeader>
...
// Form data
const formData = reactive<PatientUpdate>({
    name: '',
    birth_date: '',
    gender: 'male',
    height: undefined,
...
const loadPatientData = () => {
    if (props.patient) {
        Object.assign(formData, {
            ...
            comorbidities: props.patient.comorbidities || [],
            allergies: props.patient.allergies || [],
            assigned_doctor_id: props.patient.assigned_doctor_id,
            responsible_name: props.patient.responsible_name,
            responsible_phone: props.patient.responsible_phone,
            status: props.patient.status,
        });

        comorbiditiesText.value = props.patient.comorbidities?.join(', ') || '';
        allergiesText.value = props.patient.allergies?.join(', ') || '';
    }
};
...
const onSubmit = async () => {
    try {
        ...
        Object.keys(processedData).forEach((key) => {
            const value = processedData[key as keyof PatientUpdate];
            if (value === '' || value === undefined) {
                delete processedData[key as keyof PatientUpdate];
            }
        });

        const success = await patientsStore.updatePatient(props.patient.id, processedData);

        if (success) {
            // Find updated patient and emit
            const updatedPatient = patientsStore.patients.find((p) => p.id === props.patient.id);
            if (updatedPatient) {
